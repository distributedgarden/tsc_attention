...
class MultiheadAttentionLSTMFCN(nn.Module):
    """
    Description:
        - A Hybrid Long Short-Term Memory Fully Convolutional Network (LSTM-FCN) with a Multi-Head Attention mechanism.
        - This model enhances the LSTM-FCN by incorporating a multi-head attention layer that helps focus on specific parts of the input sequence.

    Attributes:
        - lstm (nn.LSTM): LSTM layer for sequential data processing.
        - attention (nn.MultiheadAttention): Multi-head attention layer for focusing on important features.
        - dropout (nn.Dropout): Dropout layer for regularization.
        - conv1, conv2, conv3 (nn.Conv1d): Convolutional layers for feature extraction.
        - bn1, bn2, bn3 (nn.BatchNorm1d): Batch normalization layers corresponding to each convolutional layer.
        - global_avg_pooling (nn.AdaptiveAvgPool1d): Global average pooling layer for feature aggregation.
    ...
    def __init__(
        ...
        super(MultiheadAttentionLSTMFCN, self).__init__()
        self.lstm = nn.LSTM(input_size, hidden_size, batch_first=True)
